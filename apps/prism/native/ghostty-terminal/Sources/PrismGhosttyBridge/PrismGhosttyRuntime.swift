...
enum PrismGhosttyError: Error {
  case configCreationFailed
  case appCreationFailed
  case surfaceCreationFailed
}

final class PrismGhosttyRuntime {
  private(set) var app: ghostty_app_t?
  private let callback: PrismGhosttyEventCallback
  private let callbackUserdata: UnsafeMutableRawPointer?
  private var active = true
  private var desiredFocus = false
  private var activationObservers: [NSObjectProtocol] = []

  init(callback: @escaping PrismGhosttyEventCallback, userdata: UnsafeMutableRawPointer?) throws {
    app = nil
    self.callback = callback
    callbackUserdata = userdata

    guard let config = ghostty_config_new() else {
      throw PrismGhosttyError.configCreationFailed
    }
    ...
    var runtimeConfig = ghostty_runtime_config_s(
      ...
      wakeup_cb: { userdata in PrismGhosttyRuntime.wakeup(userdata) },
      action_cb: { app, target, action in
        guard let app else { return false }
        return PrismGhosttyRuntime.action(app, target: target, action: action)
      },
      read_clipboard_cb: { userdata, location, state in
        PrismGhosttyRuntime.readClipboard(userdata, location: location, state: state)
      },
      confirm_read_clipboard_cb: { userdata, string, state, request in
        PrismGhosttyRuntime.confirmReadClipboard(
          userdata, string: string, state: state, request: request
        )
      },
      write_clipboard_cb: { userdata, location, content, count, confirm in
        PrismGhosttyRuntime.writeClipboard(
          userdata, location: location, content: content, count: count, confirm: confirm
        )
      },
      close_surface_cb: { userdata, processAlive in
        PrismGhosttyRuntime.closeSurface(userdata, processAlive: processAlive)
      }
    )

    guard let created = ghostty_app_new(&runtimeConfig, config) else {
      throw PrismGhosttyError.appCreationFailed
    }
    app = created
    desiredFocus = true
    syncFocus()
  ...
  private static func runtime(from userdata: UnsafeMutableRawPointer?) -> PrismGhosttyRuntime? {
    guard let userdata else { return nil }
    return Unmanaged<PrismGhosttyRuntime>.fromOpaque(userdata).takeUnretainedValue()
  }
  ...
  private static func action(
    ...
    let view = Unmanaged<PrismGhosttySurfaceView>.fromOpaque(userdata).takeUnretainedValue()
  ...
  private static func surface(from userdata: UnsafeMutableRawPointer?) -> ghostty_surface_t? {
    guard let userdata else { return nil }
    return Unmanaged<PrismGhosttySurfaceView>.fromOpaque(userdata).takeUnretainedValue().surface
  }
  ...
  private static func confirmReadClipboard(
    ...
    // Confirmation-requiring OSC 52 reads remain denied until Prism owns a
