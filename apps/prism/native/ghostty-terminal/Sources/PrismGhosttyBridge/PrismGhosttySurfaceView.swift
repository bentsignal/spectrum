...
final class PrismGhosttySurfaceView: NSView {
  let runtime: PrismGhosttyRuntime
  ...
  init(
    ...
    guard let app = runtime.app else { throw PrismGhosttyError.appCreationFailed }
    ...
    config.scale_factor = Double(parent.window?.backingScaleFactor ?? NSScreen.main?.backingScaleFactor ?? 1)
    ...
    guard let created else { throw PrismGhosttyError.surfaceCreationFailed }
    surface = created
  ...
  private func syncFocus() {
    guard let surface else { return }
    let ownsFocus = desiredVisible && !isHidden && NSApp.isActive
      && window?.isKeyWindow == true && window?.firstResponder === self
    ghostty_surface_set_focus(surface, ownsFocus)
  }
  ...
  private func sendKey(_ event: NSEvent, action: ghostty_input_action_e) {
    ...
    key.consumed_mods = Self.ghosttyModifiers(event.modifierFlags.subtracting([.command, .control]))
