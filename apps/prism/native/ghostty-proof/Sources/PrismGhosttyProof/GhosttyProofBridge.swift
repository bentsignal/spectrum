...
public func prismGhosttyGlobalInit(
  _ argc: Int32,
...
public func prismGhosttyRuntimeCreate() -> UnsafeMutableRawPointer? {
...
@_cdecl("prism_ghostty_surface_set_frame")
public func prismGhosttySurfaceSetFrame(
...
@_cdecl("prism_ghostty_surface_set_focus")
public func prismGhosttySurfaceSetFocus(_ raw: UnsafeMutableRawPointer?, _ focused: Bool) {
  dispatchPrecondition(condition: .onQueue(.main))
  guard let raw else { return }
  let view = Unmanaged<GhosttyProofSurfaceView>.fromOpaque(raw).takeUnretainedValue()
  if focused {
