...
use crate::{
    Command, Document, FontAsset, FontEmbeddingPermission, LayerKind, RenderRegion,
    VerifiedFontSource, font_usage::analyze_font_usage_with_source, render_document,
...
fn create_optimized_font_copy_before_publish(
    ...
    let mut cleanup = TemporaryProject::new(temporary.clone());
    let mut document = root_document(&source_store, revisions[0].id)?;
    ...
    spectrum_revisions::publish_noreplace(&temporary, &output)
        .with_context(|| format!("could not publish optimized copy {}", output.display()))?;
    cleanup.disarm();
...
fn validate_copy_store(store: &RevisionStore, expected_revisions: usize) -> Result<()> {
    let info = store.project_info()?;
    if info.application_id != APPLICATION_ID {
    ...
    for revision in revisions {
        let plan = store.replay_plan(revision.id, &PrismCompatibility)?;
        if plan.snapshot_revision != revision.id || !plan.steps.is_empty() {
