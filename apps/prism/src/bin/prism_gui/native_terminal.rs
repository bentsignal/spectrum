...
pub(super) struct NativeSurfacePresentation {
    pub(super) x: f64,
    pub(super) y: f64,
    pub(super) width: f64,
...
mod macos {
    ...
    impl DynamicLibrary {
        fn open(path: &Path) -> Result<Self> {
            ...
            // dyld to resolve the complete bridge before Prism uses any symbol.
    ...
    impl ReadyHost {
        fn load(creation: &eframe::CreationContext<'_>) -> Result<Self> {
            let parent_nsview = match creation
                .window_handle()
                .context("Prism's native window handle is unavailable")?
                .as_raw()
            {
                RawWindowHandle::AppKit(handle) => handle.ns_view,
                _ => bail!("Prism did not receive an AppKit window handle"),
            };
            let library_path = bridge_path()?;
        ...
        fn ensure_surface(&mut self, session_id: u64, context: &TerminalContext) -> Result<()> {
            ...
            let working_directory =
                CString::new(context.working_directory().as_os_str().as_encoded_bytes())
                    .context("terminal working directory contains an interior NUL")?;
    ...
    impl NativeTerminalHost {
        pub(crate) fn from_environment(creation: &eframe::CreationContext<'_>) -> Self {
            if !experiment_requested(std::env::var_os("PRISM_EXPERIMENTAL_GHOSTTY").as_deref()) {
                return Self {
                    state: HostState::Disabled,
                };
            }
            let state = match ReadyHost::load(creation) {
        ...
        pub(crate) fn fallback_diagnostic(&self) -> Option<&str> {
            match &self.state {
        ...
        pub(crate) fn reset(&mut self, session_id: u64) {
            if let HostState::Ready(host) = &mut self.state {
                host.objects.remove_surface(session_id);
            }
        }

        pub(crate) fn edit(&self, session_id: u64, action: NativeEditAction) -> Result<()> {
            match &self.state {
        ...
        pub(crate) fn request_close(&self, session_id: u64) -> Result<()> {
            match &self.state {
        ...
        pub(crate) fn retain_sessions(&mut self, session_ids: &BTreeSet<u64>) {
            if let HostState::Ready(host) = &mut self.state {
                host.retain_sessions(session_ids);
            }
        }

        pub(crate) fn poll(&mut self) -> Vec<NativeTerminalEvent> {
            match &mut self.state {
    ...
    fn bridge_path() -> Result<PathBuf> {
        if let Some(path) = std::env::var_os("PRISM_GHOSTTY_BRIDGE") {
            return Ok(PathBuf::from(path));
        }
        let executable = std::env::current_exe().context("could not locate Prism executable")?;
        let macos = executable
            .parent()
            .context("Prism executable has no containing directory")?;
        Ok(macos
            .parent()
            .context("Prism executable is not inside an app bundle")?
            .join("Frameworks")
    ...
    fn resolve_api_table(symbols: &impl SymbolSource) -> Result<BridgeApi> {
        ...
        let abi_version = load!(
            "prism_ghostty_bridge_abi_version\0",
        ...
        Ok(BridgeApi {
            global_init: load!(
                "prism_ghostty_global_init\0",
                unsafe extern "C" fn() -> c_int
            ),
            runtime_create: load!(
                "prism_ghostty_runtime_create\0",
                unsafe extern "C" fn(EventCallback, *mut c_void) -> RuntimeHandle
            ),
            runtime_tick: load!(
                "prism_ghostty_runtime_tick\0",
                unsafe extern "C" fn(RuntimeHandle)
            ),
            runtime_set_focus: load!(
                "prism_ghostty_runtime_set_focus\0",
                unsafe extern "C" fn(RuntimeHandle, bool)
            ),
            runtime_destroy: load!(
                "prism_ghostty_runtime_destroy\0",
                unsafe extern "C" fn(RuntimeHandle)
            ),
            surface_create: load!(
                "prism_ghostty_surface_create\0",
                unsafe extern "C" fn(
                    RuntimeHandle,
                    *mut c_void,
                    u64,
    ...
    mod tests {
        ...
        impl SymbolSource for FakeSymbols {
            fn symbol(&self, name: &'static CStr) -> Result<*mut c_void> {
                ...
                if name == "prism_ghostty_bridge_abi_version" {
        ...
        fn missing_required_symbol_falls_back_to_portable() {
            let error = load_api_table(
                &fake_loader(false, Some("prism_ghostty_runtime_create"), fake_abi_v1),
                Path::new("/fake/bridge"),
            )
            .err()
            .expect("API resolution must reject a missing required symbol");
            assert_portable_fallback(error, "prism_ghostty_runtime_create");
        }
...
#[cfg(all(target_os = "macos", feature = "ghostty-terminal"))]
pub(super) use macos::{NativeEditAction, NativeTerminalEvent, NativeTerminalHost};
