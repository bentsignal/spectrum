    ...
    fn new() -> Self {
        ...
        for collision in 0..32_u8 {
            let path = std::env::temp_dir().join(format!(
                "prism-mixed-raster-snapshot-{}-{stamp}-{unique}-{collision}",
                std::process::id()
            ));
            match std::fs::create_dir(&path) {
                Ok(()) => return Self(path),
                Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => continue,
                Err(error) => panic!("could not create mixed-raster test directory: {error}"),
            }
        }
        panic!("could not allocate a unique mixed-raster test directory");
    }
...
#[test]
fn concurrent_test_directories_are_process_unique() {
