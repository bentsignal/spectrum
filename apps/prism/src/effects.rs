...
impl ShapeGradient {
    ...
    fn sample(&self, normalized_x: f32, normalized_y: f32) -> [u8; 4] {
        ...
        let amount = ((projection - start.position) / span).clamp(0.0, 1.0);
...
pub(crate) fn drop_shadow_alpha(
    ...
    for (unit_x, unit_y, weight) in KERNEL {
