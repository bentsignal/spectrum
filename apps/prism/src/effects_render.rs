...
use crate::{
    ...
    render::{RegionRenderStats, RenderRegion, composite_blended_pixel},
};
...
fn masked_source_alpha(source: &RgbaImage, layer: &Layer, x: i64, y: i64) -> u8 {
    ...
    let normalized_x = x as f32 / source.width().max(1) as f32;
    let normalized_y = y as f32 / source.height().max(1) as f32;
