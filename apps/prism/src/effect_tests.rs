...
fn durable_effect_edits_use_the_version_three_operation_envelope() {
    ...
    connection
        .execute(
            "UPDATE operation_payloads SET version = 1 WHERE version = 3",
            [],
        )
        .unwrap();
    drop(connection);
    let error = match Workspace::open(&project) {
        Ok(_) => panic!("legacy operation envelope accepted version-three effect commands"),
        Err(error) => error,
    };
    assert!(
        error
            .to_string()
            .contains("contains commands requiring version 3")
    );
