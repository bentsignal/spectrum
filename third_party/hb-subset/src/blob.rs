use std::{ffi::c_char, marker::PhantomData, ops::Deref, path::Path, ptr::null_mut, slice};
...
impl Blob<'static> {
    ...
    pub fn from_file(path: impl AsRef<Path>) -> Result<Self, AllocationError> {
        // The upstream wrapper passes an OS path through HarfBuzz's narrow C
        // file API and therefore only compiles on Unix. Let Rust handle native
        // path encoding, then ask HarfBuzz to own an immediate copy. This also
        // keeps the returned blob independent of the temporary Vec.
        let bytes = std::fs::read(path).map_err(|_| AllocationError)?;

        let blob = unsafe {
            sys::hb_blob_create_or_fail(
                bytes.as_ptr() as *const c_char,
                bytes.len().try_into().map_err(|_| AllocationError)?,
                sys::hb_memory_mode_t_HB_MEMORY_MODE_DUPLICATE,
                null_mut(),
                None,
            )
        };
