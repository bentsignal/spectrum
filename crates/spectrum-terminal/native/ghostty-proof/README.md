# Spectrum Ghostty proof of life

This is a standalone macOS harness for testing Ghostty's full embedded terminal
surface before Spectrum adopts any production integration. It does not change the
Spectrum applications, Cargo manifests, or the portable `spectrum-terminal` backend.
...
[`ghostty-proof.lock`](../../../../packaging/spectrum-terminal/macos/ghostty-proof.lock)
...
- the universal macOS target, macOS 13.0 minimum deployment target, Spectrum
...
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
  bash scripts/build-spectrum-ghostty-macos.sh
```
...
Symbol validation emits a bounded `spectrum-ghostty-symbol-check` record before a
...
```text
target/ghostty-proof/dist/SpectrumGhosttyProof.app
...
```text
SpectrumGhosttyProof.app/Contents/Resources/terminfo/78/xterm-ghostty
```
...
```sh
open target/ghostty-proof/dist/SpectrumGhosttyProof.app
```
...
or app/eframe child-view and overlay behavior; those paths are deliberately
incomplete.
...
```sh
bash -n scripts/build-spectrum-ghostty-macos.sh
plutil -lint crates/spectrum-terminal/native/ghostty-proof/Info.plist
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
  xcrun swift-format lint --strict --recursive \
  crates/spectrum-terminal/native/ghostty-proof/Package.swift \
  crates/spectrum-terminal/native/ghostty-proof/Sources
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
  xcrun swiftc -frontend -parse \
  crates/spectrum-terminal/native/ghostty-proof/Package.swift \
  crates/spectrum-terminal/native/ghostty-proof/Sources/SpectrumGhosttyProof/*.swift
git diff --check
```
