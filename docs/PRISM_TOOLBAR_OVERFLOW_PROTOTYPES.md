...
`Gradient · Kind · N stops` control opens a bounded vertical editor popover.
That popover retains geometry, spread, every stop, and each stop's nested visual
...
- Narrow: tool identity and Tools & Actions remain fixed; every contextual
  ...
  is a nested, explicitly labeled editor popover.
...
- The full gradient editor remains in its popover instead of expanding a row
