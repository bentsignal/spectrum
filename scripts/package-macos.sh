...
verify_chain_sources() {
  ...
  [[ "$(sha256_file "$repo_root/scripts/package-macos.sh")" == "$chain_package_sha" ]]
...
if [[ "$ghostty_enabled" == true ]]; then
  ...
  chain_package_sha="$(sha256_file "$repo_root/scripts/package-macos.sh")"
...
if [[ "$ghostty_enabled" == true ]]; then
  cargo build --release --locked -p lumen-photo --bins --features ghostty-terminal
else
...
if [[ "$ghostty_enabled" == true ]]; then
  ...
  for binary in "$bundle/Contents/MacOS/lumen-gui" "$bundle/Contents/MacOS/lumen"; do
