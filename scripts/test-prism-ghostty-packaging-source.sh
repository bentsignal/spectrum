...
cleanup() {
  if [[ -n "${private_package_root:-}" && -d "$private_package_root" \
    ...
    case "$private_package_root" in
      "$repo_root"/target/prism-ghostty-package.*) rm -rf -- "$private_package_root" ;;
    esac
  fi
...
PRISM_GHOSTTY_BRIDGE_ABI_VERSION=1
...
private_package_root="$(mktemp -d "$repo_root/target/prism-ghostty-package.XXXXXX")"
...
spec = importlib.util.spec_from_file_location("prism_bounded_runner", sys.argv[1])
...
sdk_verify=(python3 "$sdk_validator" "$sdk_fixture" macosx15.2 15.2 \
  ...
  "$sdk_tree_hasher" "$sdk_uid" "$sdk_gid" prism-sdk-v1)
"${sdk_verify[@]}"
...
expect_failure "SDK node ownership mismatch" python3 "$sdk_validator" \
  "$sdk_fixture" macosx15.2 15.2 "$sdk_tree_sha" "$sdk_settings_sha" \
  "$sdk_libsystem_sha" "$sdk_libcxx_sha" "$sdk_tree_hasher" \
  "$((sdk_uid + 1))" "$sdk_gid" prism-sdk-v1
...
xcrun_result="$(PRISM_GHOSTTY_MACOS_SDK_ROOT="$sdk_fixture" \
  bash "$xcrun_shim" --sdk macosx --show-sdk-path)"
[[ "$xcrun_result" == "$sdk_fixture" ]]
expect_failure "" env PRISM_GHOSTTY_MACOS_SDK_ROOT="$sdk_fixture" \
...
actual_clang="$(DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer \
  PRISM_GHOSTTY_MACOS_SDK_ROOT="$sdk_fixture" bash "$xcrun_shim" --find clang)"
...
mode = os.environ.get("PRISM_GHOSTTY_TEST_MODE", "ok")
symbols = [
...
if tool == "otool":
    if sys.argv[1] == "-D":
        print(sys.argv[2])
        print("@rpath/wrong.dylib" if mode == "install-name" else "@rpath/libPrismGhosttyBridge.dylib")
    elif sys.argv[1] == "-L":
        print(sys.argv[2] + ":")
...
expect_failure "symlink or non-canonical component" env \
  PATH="$fixture/shims:/usr/bin:/bin" PRISM_GHOSTTY_TEST_MODE=ok \
...
for test_case in \
  ...
  expect_failure "$expected" env PATH="$fixture/shims:/usr/bin:/bin" \
    PRISM_GHOSTTY_TEST_MODE="$mode" "${bridge_command[@]}"
done
expect_failure "" env PATH="$fixture/shims:/usr/bin:/bin" \
  PRISM_GHOSTTY_TEST_MODE=signature "${bridge_command[@]}"
...
expect_failure "rejects externally prepared proof roots" env \
  ...
  PRISM_GHOSTTY_PRIVATE_ROOT="$private_package_root" \
  bash "$repo_root/scripts/build-prism-ghostty-bridge-macos.sh" \
