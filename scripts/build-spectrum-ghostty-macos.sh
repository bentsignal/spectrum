...
harness_source="$repo_root/crates/spectrum-terminal/native/ghostty-proof"
...
xcrun_shim_source="$repo_root/scripts/spectrum-ghostty-xcrun-shim.sh"
...
[[ "$bridge_abi_version" =~ ^[1-9][0-9]*$ ]] \
  || die "invalid Spectrum Ghostty bridge ABI version: $bridge_abi_version"
...
actual_bridge_abi_version="$(awk \
  ...
  "$repo_root/crates/spectrum-terminal/native/ghostty-bridge/include/spectrum_ghostty_bridge.h")" \
...
(
  ...
  run_bounded 3600 env \
    ...
    SPECTRUM_GHOSTTY_MACOS_SDK_ROOT="$clt_macos_sdk_root" \
...
harness_bin_dir="$(xcrun swift build \
  ...
  --arch "$swift_arch" \
  --show-bin-path)"
harness_executable="$harness_bin_dir/SpectrumGhosttyProof"
[[ -x "$harness_executable" ]] \
  || die "Swift build did not produce expected harness: $harness_executable"

bundle="$dist_root/SpectrumGhosttyProof.app"
bundle_staging="$(mktemp -d "$storage_root/bundle.XXXXXX")/SpectrumGhosttyProof.app"
mkdir -p "$bundle_staging/Contents/MacOS" "$bundle_staging/Contents/Resources"
install -m 0755 "$harness_executable" "$bundle_staging/Contents/MacOS/SpectrumGhosttyProof"
