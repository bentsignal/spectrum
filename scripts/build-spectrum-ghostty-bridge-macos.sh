...
bridge_source="$repo_root/crates/spectrum-terminal/native/ghostty-bridge"
...
verify_reviewed_inputs_unchanged() {
  [[ "$(sha256_file "$repo_root/scripts/build-spectrum-ghostty-bridge-macos.sh")" == \
  ...
  [[ "$(sha256_file "$repo_root/scripts/build-spectrum-ghostty-macos.sh")" == \
...
private_root="${SPECTRUM_GHOSTTY_PRIVATE_ROOT:-}"
...
bridge_abi_version="$(lock_value SPECTRUM_GHOSTTY_BRIDGE_ABI_VERSION)"
...
reviewed_consumer_script_sha="$(sha256_file "$repo_root/scripts/build-spectrum-ghostty-bridge-macos.sh")"
reviewed_proof_script_sha="$(sha256_file "$repo_root/scripts/build-spectrum-ghostty-macos.sh")"
...
python3 "$metadata_validator" attestation "$proof_attestation" \
  ...
  "SPECTRUM_GHOSTTY_BRIDGE_ABI_VERSION=$bridge_abi_version" \
...
[[ "$(attestation_value SPECTRUM_GHOSTTY_BRIDGE_ABI_VERSION)" == "$bridge_abi_version" ]] \
  || die "proof attestation bridge ABI mismatch"

library_sha="${SPECTRUM_GHOSTTY_SEAL_LIBRARY_SHA256:-}"
header_sha="${SPECTRUM_GHOSTTY_SEAL_HEADER_SHA256:-}"
xcframework_info_sha="${SPECTRUM_GHOSTTY_SEAL_XCFRAMEWORK_INFO_SHA256:-}"
xcframework_tree_sha="${SPECTRUM_GHOSTTY_SEAL_XCFRAMEWORK_TREE_SHA256:-}"
sentinel_sha="${SPECTRUM_GHOSTTY_SEAL_SENTINEL_SHA256:-}"
resources_tree_sha="${SPECTRUM_GHOSTTY_SEAL_RESOURCES_TREE_SHA256:-}"
license_sha="${SPECTRUM_GHOSTTY_SEAL_LICENSE_SHA256:-}"
for digest_name in \
  library_sha \
  header_sha \
...
actual_bridge_abi_version="$(awk \
  '$1 == "#define" && $2 == "SPECTRUM_GHOSTTY_BRIDGE_ABI_VERSION" { print $3; found = 1; exit } END { if (!found) exit 1 }' \
  "$bridge_source/include/spectrum_ghostty_bridge.h")" \
  || die "could not read Spectrum Ghostty bridge ABI version"
[[ "$actual_bridge_abi_version" == "$bridge_abi_version" ]] \
  || die "Spectrum Ghostty bridge ABI does not match the reviewed lock"
...
binary_directory="$(xcrun swift build \
  ...
  "${swift_prefix_flags[@]}" \
  --show-bin-path)"
bridge="$binary_directory/libSpectrumGhosttyBridge.dylib"
[[ -f "$bridge" ]] || die "Swift build did not produce $bridge"
install_name_tool -id "@rpath/libSpectrumGhosttyBridge.dylib" "$bridge"
...
install -m 0755 "$bridge" "$output_candidate/libSpectrumGhosttyBridge.dylib"
...
bash "$bridge_verifier" "$output_candidate/libSpectrumGhosttyBridge.dylib" \
